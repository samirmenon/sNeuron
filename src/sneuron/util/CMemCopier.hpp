...
/***
 * A template class that can memcpy its member
...
private:
  ...
  const T* mem_;
  /***
   * The buffer to which the data is copied.
   */
  std::vector<T*> buf_;
  /***
   * Initialized
...
public:
  ...
  /***
   * Initializes a mem copier
   */
  bool init(const T* arg_mem);

  /***
   * Registers a buffer with the mem copier.
   ...
   * Argument: The pointer to the buffer to be removed
