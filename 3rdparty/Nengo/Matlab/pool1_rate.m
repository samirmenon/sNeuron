function [mean_time std_time mean_val std_val] = pool1_rate( val )
%UNTITLED2 Summary of this function goes here
%   Detailed explanation goes here
str = strcat('C:\Users\Srish\Desktop\NewestSimulations\1pool_Rate\Value',num2str(val),'\');
    ...
    for n=100:100:400
        ...
        for trial = 1:5
            ...
            for i=2:length(data_filtered)
                if(abs((data_filtered(i) - (val*0.9)))<= 0.01) %change 0.41 to 0.11 for 1 pool
                    file
                    time = [time b(i)]
                    break;
                end;
...
legend('d=50','d=100','d=200','d=300');
hold on;
ss1 = strcat('Plots_Newest/Rate1_Value_',num2str(val),'.tif');
ss2 = strcat('Plots_Newest/Rate1_Value_',num2str(val),'.eps');
saveas(h,ss1);
saveas(h,ss2);
close;
end
